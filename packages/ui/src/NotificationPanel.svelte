<script lang="ts">
  ...
  function onKeydown(e: KeyboardEvent) {
    ...
    if (e.key === 'Tab' && panelRef) {
      const focusables = panelRef.querySelectorAll<HTMLElement>('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');
...
{#if show}
  ...
  <!-- Notification Panel: bottom drawer on mobile, side panel on desktop -->
  <div class="fixed inset-x-0 bottom-0 sm:inset-auto sm:top-16 sm:right-4 sm:w-96 sm:max-w-[calc(100vw-2rem)] z-50 {className}">
    <div
      bind:this={panelRef}
      role="dialog"
      aria-modal="true"
      ...
      tabindex="-1"
      onkeydown={onKeydown}
      class="bg-white/95 backdrop-blur-xl rounded-t-2xl sm:rounded-2xl shadow-2xl ring-1 ring-black/5 
      border border-white/20 overflow-hidden h-[70dvh] sm:h-auto sm:max-h-[80vh] flex flex-col"
    >
      
      <!-- Header -->
      <div class="flex items-center justify-between px-4 pt-[max(1rem,env(safe-area-inset-top))] pb-4 border-b border-gray-100/50 bg-white/80 sticky top-0 backdrop-blur supports-[backdrop-filter]:bg-white/60">
        <div>
          <h3 id="notification-panel-title" class="font-semibold text-gray-900">{translations.title || 'Notifications'}</h3>
          {#if unreadCount > 0}
...
{translations.markAllRead || 'Mark all read'}
          ...
          <button 
            onclick={onClose}
            class="p-1 text-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-300"
          ...
          <div class="p-2">
            {#each notifications as notification}
              <button
                ...
                class="w-full text-left p-3 rounded-xl transition-colors
      ...
      {#if notifications.length > 0}
        <div class="px-4 pt-3 pb-[max(1rem,env(safe-area-inset-bottom))] sm:pb-3 border-t border-gray-100/50 bg-white/80 sticky bottom-0 backdrop-blur supports-[backdrop-filter]:bg-white/60">
          <Button 
            variant="ghost" 
            size="sm" 
            href="/notifications"
