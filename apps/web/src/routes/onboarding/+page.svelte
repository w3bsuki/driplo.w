    ...
    {#snippet children()}
      ...
      <div class="flex justify-center space-x-3 mb-8">
        {#each Array(totalSteps) as _, i}
          <div class="h-2 rounded-full transition-all duration-200 motion-reduce:transition-none {i + 1 <= step ? 'bg-black w-8' : 'bg-gray-200 w-2'}"></div>
        {/each}
      </div>
      ...
      <div class="flex space-x-4">
        <Button
          onclick={nextStep}
          disabled={!canProceed()}
          class="flex-1 bg-black text-white sm:hover:bg-gray-800"
        >
          Continue
        </Button>
      </div>
    ...
    {#snippet children()}
      ...
      <div class="flex space-x-4">
        ...
        <Button
          ...
          disabled={submitting}
          loading={submitting}
          class="flex-1 bg-green-600 text-white sm:hover:bg-green-700"
        >
