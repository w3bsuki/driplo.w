<script lang="ts">
  ...
  let socialLinks = $state<SocialLink[]>(data.profile?.social_links || []);
  ...
  function removeSocialLink(index: number) {
    socialLinks = socialLinks.filter((_: SocialLink, i: number) => i !== index);
  }
  ...
  async function saveProfile() {
    ...
    const { error } = await data.supabase
      ...
      .update({
        ...
        social_links: socialLinks.filter((link: SocialLink) => link.url.trim())
      })
      .eq('id', data.user.id);
...
<div class="min-h-screen bg-gray-50 pb-[max(5rem,env(safe-area-inset-bottom))] sm:pb-0">
  <Header />
  ...
  <div class="max-w-2xl mx-auto p-4">
    <div class="bg-white rounded-lg p-6">
      <div class="flex items-center justify-between mb-6">
        ...
        <a href="/profile/{data.profile?.username || data.user?.id}" rel="noopener" class="text-sm text-gray-600 hover:underline select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]">{i18n.profile_cancel()}</a>
      </div>

      <div class="space-y-6">
        <!-- Avatar -->
        <div>
          <h2 class="block text-sm font-medium mb-2">{i18n.profile_avatar()}</h2>
          ...
          <div class="relative mb-4">
            <div class="flex space-x-3 overflow-x-auto scrollbar-hide pb-2 select-none [touch-action:pan-x] [-webkit-tap-highlight-color:transparent]">
              {#each avatars as avatar}
                <button
                  type="button"
                  onclick={() => avatarUrl = avatar}
                  class="relative group flex-shrink-0 select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]"
                  aria-label={`Select avatar ${avatar}`}
                >
                  <div class="relative w-16 h-16 overflow-hidden rounded-xl border-2 transition-all duration-200 motion-reduce:transition-none {avatarUrl === avatar ? 'border-black ring-2 ring-black/20' : 'border-gray-200 hover:border-gray-300'}">
                    <img src={avatar} alt="Avatar option" class="w-full h-full object-cover" loading="lazy" decoding="async" />
        ...
        <div>
          <label class="block text-sm font-medium mb-1" for="bio-textarea">{i18n.profile_bio()}</label>
          <textarea
            bind:value={bio}
            rows="3"
        ...
        <!-- Social Links -->
        <div>
          <h2 class="block text-sm font-medium mb-2">{i18n.profile_socialLinks()}</h2>
          {#each socialLinks as link, i}
            <div class="flex gap-2 mb-2">
              <select bind:value={link.type} class="px-3 py-2 border border-gray-300 rounded-lg select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]">
              ...
              <Button onclick={() => removeSocialLink(i)} variant="outline" size="sm" class="select-none">
                <span aria-hidden="true">×</span>
                <span class="sr-only">Remove link</span>
              </Button>
            </div>
          {/each}
          <Button onclick={addSocialLink} variant="outline" class="w-full select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]">{i18n.profile_addLink()}</Button>
        </div>
        ...
        <div class="flex justify-end space-x-2">
          <Button
            ...
            class="select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]"
          >
