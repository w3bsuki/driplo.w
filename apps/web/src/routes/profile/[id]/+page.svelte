<script lang="ts">
  ...
  // Optional social links if present in extended profile
  const socialLinks = (data.profile as any)?.social_links as { type: string; url: string }[] | undefined;

  const isNewSeller = (() => {
    const created = new Date(data.profile.created_at).getTime();
    const daysSince = (Date.now() - created) / (1000 * 60 * 60 * 24);
    return (data.profile.sales_count || 0) < 5 && daysSince <= 30;
  })();
...
<div class="min-h-screen bg-white pb-[max(5rem,env(safe-area-inset-bottom))] sm:pb-0">
  <Header />
  ...
  <div class="px-4 py-6 overflow-visible">
      ...
      <div class="flex-1 min-w-0">
        ...
        {#if data.isOwnProfile}
          <div class="flex space-x-2">
            <a href="/profile/edit" class="flex-1 select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]">
              <Button variant="outline" size="sm" class="w-full text-sm">{i18n.profile_editProfile()}</Button>
            </a>
            <a href="/dashboard" class="flex-1 select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]">
    ...
    <div class="mt-3">
      <div class="flex items-center space-x-2 flex-wrap gap-y-1">
        ...
        {#if data.profile.is_verified}
          <BrandBadge size="sm" verified={true} />
        {/if}
        
        <!-- New Seller Badge (first 30 days + low sales) -->
        {#if isNewSeller}
      ...
      {#if socialLinks && socialLinks.length > 0}
        <div class="flex items-center space-x-2 mt-2">
          {#each socialLinks as link}
            <a href={link.url} target="_blank" rel="noopener" class="text-blue-600 text-xs hover:underline select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]" aria-label={`Open ${link.type} link`}>
  ...
  <!-- Tab Navigation -->
  <div class="border-b border-gray-200 sticky top-14 sm:top-16 z-20 bg-white" role="tablist" aria-label="Profile sections">
    <div class="flex">
      <button
        onclick={() => activeTab = 'posts'}
        class="flex-1 py-3 text-center border-b-2 transition-colors motion-reduce:transition-none select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent] {activeTab === 'posts' ? 'border-black' : 'border-transparent'}"
        role="tab"
      ...
      <button
        ...
        class="flex-1 py-3 text-center border-b-2 transition-colors motion-reduce:transition-none select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent] {activeTab === 'reviews' ? 'border-black' : 'border-transparent'}"
        role="tab"
      ...
      <button
        ...
        class="flex-1 py-3 text-center border-b-2 transition-colors motion-reduce:transition-none select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent] {activeTab === 'about' ? 'border-black' : 'border-transparent'}"
        role="tab"
  ...
  <div class="px-4 py-4">
    {#if activeTab === 'posts'}
      <div id="tab-panel-posts" role="tabpanel" aria-labelledby="tab-posts">
      <!-- Products Grid -->
        ...
        <div class="grid grid-cols-3 gap-1">
          {#each data.products as product}
            <a
              ...
              class="aspect-square bg-gray-100 rounded overflow-hidden block select-none [touch-action:manipulation] [-webkit-tap-highlight-color:transparent]"
              >
              <img 
    ...
    {:else if activeTab === 'reviews'}
      <div id="tab-panel-reviews" role="tabpanel" aria-labelledby="tab-reviews">
      <!-- Reviews -->
