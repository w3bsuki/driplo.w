<script lang="ts">
  ...
  function handleBackdropKeydown(e: KeyboardEvent) {
    // Support closing on Escape, Enter, or Space when backdrop is focused
...
{#if seller}
  <!-- Backdrop -->
  <div 
    class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 pb-[max(6rem,env(safe-area-inset-bottom))] sm:pb-4"
    role="button"
    aria-label="Close"
    tabindex="0"
      ...
      <div class="flex-1 overflow-y-auto px-4 sm:px-6 py-3">
        ...
        <div class="grid grid-cols-2 gap-2 sm:gap-3">
          {#each products.length > 0 ? products : mockSellerProducts as product}
            <div class="cursor-pointer" onclick={() => viewProduct(product.id)} role="button" tabindex="0" onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); viewProduct(product.id); } }}>
              <div class="relative">
                <img 
